...
#define LED_SC27XX_BLUE			"/sys/class/leds/sc27xx:blue/brightness"
#define LED_SC27XX_RED			"/sys/class/leds/sc27xx:red/brightness"
#define LED_SC27XX_GREEN		"/sys/class/leds/sc27xx:green/brightness"
//------------------------------------------------------------------------------
...
{
	struct stat stt;
	char *blue_path = LED_SC27XX_BLUE;
	char *green_path = LED_SC27XX_GREEN;
	char *red_path = LED_SC27XX_RED;
	char *ledn = blue_path;

	if( !(stat("/sys/class/leds/sc27xx:red", &stt) == 0) ) {
		INFMSG("[light] stat /sys/class/leds/sc27xx:red failed\n");
		red_path = LED_RED;
		if( !(stat("/sys/class/leds/red", &stt) == 0) ) {
			INFMSG("[light] stat /sys/class/leds/red failed\n");
			return -1;
		}
	}
	if( !(stat("/sys/class/leds/sc27xx:green", &stt) == 0) ) {
		INFMSG("[light] stat /sys/class/leds/sc27xx:green failed\n");
		green_path = LED_GREEN;
		if( !(stat("/sys/class/leds/green", &stt) == 0) ) {
			INFMSG("[light] stat /sys/class/leds/green failed\n");
			return -1;
		}
	}
	if( !(stat("/sys/class/leds/sc27xx:blue", &stt) == 0) ) {
		INFMSG("[light] stat /sys/class/leds/sc27xx:blue failed\n");
		blue_path = LED_BLUE;
		if( !(stat("/sys/class/leds/blue", &stt) == 0) ) {
			INFMSG("[light] stat /sys/class/leds/bule failed\n");
			return -1;
		}
	}
    ...
    if (brightness == 0) {//diff with bbat test
        if (index == 0x1f) {
            ledn = red_path;
            LedSetValue(ledn, 0);
        } else if (index == 0x2f) {
