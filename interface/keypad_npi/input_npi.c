...
{   
    ...
    if( s_fdKPD_power < 0 ) {
        INPUT_LOGE("open power keypad '%s' fail!\n", DEV_NAME_KPD);
        ...
        if( s_fdKPD_volume < 0 ) {
		INPUT_LOGE("open volume keypad '%s' fail!\n", DEV_NAME_KPD_VOLUME);
		close(s_fdKPD_power);
		return -1;
        }
    }else {
...
int inputKPDWaitKeyPress( struct kpd_info_t * info, int timeout )
{
	...
	INPUT_LOGI("key=%d, row = %d, col = %d, gio = %d\n", 
...
int inputKPDWaitKeyPress2( struct kpd_info_t * info, int timeout )
{
	...
	if (errno != EAGAIN && errno != EINTR) {
	    INPUT_LOGD("could not get event (errno=%d)", errno);
	}
    } else if ((readSize_power % sizeof(struct input_event)) != 0) {
		INPUT_LOGE("could not get event (wrong size: %d)", readSize_power);
    } else {
...
int inputTPGetPoint( int *x, int *y, int timeout )
{
    int fd[1];
    if( s_fdTP < 0 ) {
        INPUT_LOGE("touchpanel '%s' unopened!\n", DEV_NAME_TP);
        return -1;
    }
    fd[0] = s_fdTP;
...
int inputClose2( void )
{
    ...
    if( sRunning ) {
        ...
        if( pthread_cond_timedwait(&sCondExit, &sMutxExit, &to) < 0 ) {
            INPUT_LOGD("wait thread exit timeout!\n");
        }
...
int openInputDev( const char * name )
{
    ...
    for( int i = 0; i < 16; ++i ) {
        ...
        if( ioctl(fd, EVIOCGNAME(sizeof(devName)), devName) > 0 && 
            ...
            INPUT_LOGI("open '%s' OK\n", name);
            break;    
        }
        
        INPUT_LOGD("input evt name = %s, dev name = %s\n", inputEvtName, devName);
        close(fd);
        fd = -1;
    }
    ...
    if( fd >= 0 ) {
        if( fcntl(fd, F_SETFL, O_NONBLOCK) < 0 ) {
            INPUT_LOGE("fcntl: set to nonblock error: %s!\n", strerror(errno));
        }
    }
...
int pollInputDev( int fd[], int n, int timeout)
{
    ...
    if( ret <= 0 ) {
        if( 0 == ret ) {
            INPUT_LOGD("poll timeout (%d ms)\n", timeout);
        } else {
            INPUT_LOGE("poll error: %s\n", strerror(errno));
        }
        return -1;
    }
